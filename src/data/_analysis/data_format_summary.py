...
class DataFormatAnalyzer:
    ...
    def analyze_excel_file(self, file_path: Path, sample_size: int = 5) -> Dict[str, Any]:
        """
    ...
    def analyze_jsonl_file(self, file_path: Path, sample_size: int = 5) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if sample_records and isinstance(sample_records[0], dict):
                ...
                for key in first_record.keys():
                    values = [record.get(key) for record in sample_records if isinstance(
                        record, dict) and key in record]
                    if values:
                        field_analysis[key] = {
            ...
            return {
                ...
                "record_structure": "dict" if sample_records and isinstance(sample_records[0], dict) else "other",
    ...
    def analyze_json_file(self, file_path: Path, sample_size: int = 5) -> Dict[str, Any]:
        """
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if is_array:
                    ...
                    if objects:
                        ...
                        if isinstance(first_obj, dict):
                            for key in first_obj.keys():
                                values = [obj.get(key) for obj in objects if isinstance(
                                    obj, dict) and key in obj]
                                if values:
                                    field_analysis[key] = {
                                        "type": type(values[0]).__name__,
                                        "sample_values": values[:3]
                        ...
                        return {
                            ...
                            "record_structure": "dict" if isinstance(first_obj, dict) else type(first_obj).__name__,
                            "fields": field_analysis,
                        ...
                        return {
                            ...
                            "first_chunk_preview": content[:200] + "..." if len(content) > 200 else content
                        }
                    ...
                    while i < len(content) and len(sample_structure) < sample_size:
                        ...
                        if char == '"':
                            ...
                            if char == '{':
                                ...
                                brace_count -= 1
                            elif char == ',' and brace_count == 1 and current_key is not None:
                                try:
                                    value_text = content[current_value_start:i].strip(
                                    )
                                    if value_text.startswith('"') and value_text.endswith('"'):
                                        sample_value = json.loads(value_text)
                                        sample_structure[current_key] = {"type": "str", "sample_value": str(
                                            sample_value)[:100] + "..." if len(str(sample_value)) > 100 else str(sample_value)}
                                        ...
                                        sample_structure[current_key] = {
                                            "type": "list", "sample_value": f"Array with {len(parsed_array)} elements. First element: {str(parsed_array[0])[:100] + '...' if parsed_array else 'Empty array'}"}
                                    elif value_text.startswith('{'):
                                        sample_structure[current_key] = {
                                            "type": "dict", "sample_value": "Object (nested structure)"}
                                    else:
                                        parsed_value = json.loads(value_text)
                                        sample_structure[current_key] = {"type": type(
                                            parsed_value).__name__, "sample_value": str(parsed_value)}
                                except Exception as parse_e:
                                    sample_structure[current_key] = {
                                        "type": "unknown", "sample_value": f"Parse error: {str(parse_e)[:50]}"}
                                current_key = None
                                current_value_start = None
                        i += 1
                    ...
                    return {
                        ...
                        "total_top_level_keys": len(keys_found) if len(keys_found) < 10 else "10+",
                ...
                return {
                    ...
                    "first_chunk_preview": chunk[:200] + "..." if len(chunk) > 200 else chunk
                }

        except Exception as e:
            return {"error": f"Failed to analyze JSON: {str(e)}"}

    def analyze_split_file(self, file_path: Path, sample_size: int = 5) -> Dict[str, Any]:
        """
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if first_line:
                    try:
                        ...
                        result = self.analyze_jsonl_file(
    ...
    def analyze_dataset(self, dataset_path: Path) -> Dict[str, Any]:
        ...
        split_files = [f for f in files_in_dir if f.is_file()
                       and 'part_' in f.name]
        ...
        if split_files:
            ...
            for f in files_in_dir:
                if f.is_file() and any(f.name.startswith(base_name) and 'part_' not in f.name for base_name in base_names):
    ...
    def save_summary(self, output_file: str = "src/data/_analysis/data_format_summary.json"):
        """
    ...
    def print_summary(self):
        ...
        for dataset_name, dataset_info in self.summary.items():
            ...
            for file_name, file_info in dataset_info["files"].items():
                ...
                print(
                    f"   Size: {file_info.get('file_size_mb', 'Unknown')} MB")
