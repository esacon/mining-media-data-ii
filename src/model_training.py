...
def prepare_features_and_labels(data):
    ...
    for player_id in data['features']:
        features.append(list(data['features'][player_id].values()))
        labels.append(int(data['windows'][player_id]['churned']))
...
def plot_feature_importance(model, feature_names, model_name, game_number):
    ...
    plt.savefig(f'plots/feature_importance_game{game_number}_{model_name.lower().replace(" ", "_")}.png')
    plt.close()
...
def main():
    ...
    for game_number in [1, 2]:
        ...
        # Get feature names
        feature_names = list(next(iter(train_data['features'].values())).keys())
        ...
        with open(f'processed_data/metrics_game{game_number}.json', 'w') as f:
