...
def load_analysis_results(
    analysis_dir: str = "assignment-2/results/analysis/llm",
) -> pd.DataFrame:
...
def plot_model_comparison_heatmap(
    df: pd.DataFrame, metric: str = "MCC", save_path: Optional[str] = None
):
...
def plot_prompt_type_comparison(df: pd.DataFrame, save_path: Optional[str] = None):
    ...
    for i, metric in enumerate(metrics):
        ...
        grouped = df.groupby(["Model", "Prompt_Type"])[metric].mean().reset_index()
...
def save_plots(df: pd.DataFrame, output_dir: str = "assignment-2/results/llm/plots"):
    ...
    plots = [
        (
            ...
            lambda: plot_model_comparison_heatmap(
                df, save_path=output_path / "model_comparison_heatmap.png"
            ),
        ),
        (
            "prompt_type_comparison.png",
            lambda: plot_prompt_type_comparison(
...
def main():
    analysis_dir = (
        sys.argv[1] if len(sys.argv) > 1 else "assignment-2/results/analysis/llm"
    )
